...
async function fetchKoshaByCategory(cas, categoryId) {
    ...
    try {
        ...
        try {
            ...
            throw new Error('JSON 파싱 실패 (인증키 또는 응답형식 확인 필요)');
        }

        // 응답 경로 다중 처리 (공공데이터포털 표준)
...
function analyzeItems(cas, items) {
    ...
    for (const item of items) {
        ...
        if (docId.startsWith('KOSHA07') || docId.startsWith('KOSHA06')) {
            ...
            if (merged.includes('발암')) { tags.add('발암성'); cmrDetected = true; }
            if (merged.includes('변이원')) { tags.add('변이원성'); cmrDetected = true; }
            if (merged.includes('생식')) { tags.add('생식독성'); cmrDetected = true; }
        }
    ...
    const sources = {
        ...
        nier: matched.nier ? {
            ok: true, hit: true, note: '화관법 유독물질 해당'
        } : {
            ok: true, hit: false, note: '화관법 매칭 없음 (KOSHA API 기반 판정)'
        },
        nfa: matched.nfa ? {
            ok: true, hit: true, note: '소방법 지정 위험물'
        } : {
            ok: true, hit: false, note: '위험물안전관리법 매칭 없음'
        },
        cci: matched.cci ? {
            ok: true, hit: true, note: '화학물질안전관리정보 등재'
        } : {
